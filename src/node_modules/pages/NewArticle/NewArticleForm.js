...
import TextEditor from "pages/NewArticle/TextEditor.js";
...
function NewArticleForm(props) {
  ...
  const articleTextEditorRef = useRef();

  const SubmitHandler = (e) => {
    e.preventDefault();

    articleTextEditorRef.current.save();
    setIsSubmiting(true);
  }
  useEffect(() => {
    if (isSubmiting) {
      if (isAllFieldsCorrect()) {
        if (isEdit) {
          ...
          updateArticleAndRedirect(articleId);
          return;
        }
  ...
  const articleSaveHandler = (data) => {
    setArticleText(data);
  }
  const updateArticleAndRedirect = (articleId) => {
    if (articleId) {
      projectDB.ref(`articles/${articleId}/${localStorage.getItem("currentUserId")}`).set({
  ...
  const isAllFieldsCorrect = () => {
    setFieldsErrors({
      ...
      articleTextErr: !JSON.parse(articleText).blocks.find(block => block.text.length !== 0),
      articleTagsErr: !isTagsCorrect(),
    })
    if (
      articleTitle.trim().length === 0 ||
      articleDesc.trim().length === 0 ||
      !JSON.parse(articleText).blocks.find(block => block.text.length !== 0) ||
      !isTagsCorrect()
    ) {
      if (
        articleTitle.trim().length === 0 ||
  ...
  const isTagsCorrect = () => {
    ...
    const tagsArr = articleTags.trim().split(" ");
      ...
      isArticleExist ?
        isAllowed ?
          <form onSubmit={SubmitHandler} className={styles.root} noValidate autoComplete="off" >
            ...
            <fieldset
              className={styles.fieldsetStyles}
              style={{
                border: fieldsErrors.articleTextErr ? "1px solid #f44336" : "1px solid #888",
                padding: "0px 8px"
              }}
            >
              <TextEditor
                articleTextEditorRef={articleTextEditorRef}
                defaultValue={articleText}
                currentLang={currentLang}
                setArticleText={setArticleText}
                articleText={articleText}
                articleSaveHandler={articleSaveHandler}
              />
            </fieldset>
            <fieldset className={styles.fieldsetStyles}>
            ...
            <fieldset style={{ justifySelf: "center" }} className={styles.fieldsetStyles}>
              <FormControl>
                <Button size="large" type="submit" variant="contained" color="primary">
                  {isEdit ?
                    LangsTranslate[currentLang].newArticle["articleUpdate"] :
