...
const GeneralFeed = ({ currentLang, searchByTagHandler, tabName, ...props }) => {
  ...
  const setUserPageOwnerArticles = (userId) => {
    projectDB.ref(`users/${userId}/articlesref`).limitToLast(articlesAmount).on('value', (snapshot) => {
      const data = snapshot.val();
      if (!data) {
        setIsNoArticles(true);
        setArticles(null);
      ...
      setIsEnd(Object.keys(articles).length < articlesAmount);
      setArticles(getArticlesInfo(articles));
    });
  }
  ...
  useEffect(() => {
    ...
    else if (tabName === "owns") setUserPageOwnerArticles(props.match.params.id);
    else setIsNoArticles(true);
