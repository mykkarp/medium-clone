...
export default function ComposedTextField(props) {
  ...
  const isTagsCorrect = () => {
    ...
    if (articleTags.lenght === 0) return true;
    if (articleTags.split(" ").find(tag => tag[0] !== "#")) {
      setErrorMessage(formErrors.startWithHashTag);
      return false;
    }
    if (articleTags.split(" ").find(tag => (tag[0] === "#" && tag.length === 1))) {
      setErrorMessage(formErrors.afterHashTagName);
      return false;
    }
    if (articleTags.split(" ").length !== [...new Set(articleTags.split(" "))].length) {
      setErrorMessage(formErrors.dublicatTags);
      return false;
    }
    if (articleTags.split(" ").find(tag => !(tag.match(/[a-zA-Zа-яА-Я0-9-_#]/g).length === tag.length))) {
