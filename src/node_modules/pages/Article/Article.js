...
const Article = (props) => {
  ...
  const feedInfoTranslate = LangsTranslate[currentLang].Feeds;
  ...
  const getArticleInfo = (data) => {
    let articleOwnerId = "";
    for (let articleContent in data) {
      if (
    ...
    return Object.assign({}, {
      publishedDate: moment(new Date(+articleId)).format("DD.MM.YYYY"),
      key: article,
      comments: data.comments || {},
      liked: data.liked || {},
      disliked: data.disliked || {},
      articleOwnerId: articleOwnerId,
      articleText: data[articleOwnerId].articleText,
      articleTitle: data[articleOwnerId].articleTitle,
      articleDesc: data[articleOwnerId].articleDesc,
      articleTags: data[articleOwnerId].articleTags,
    });
  }
  const likeHandler = () => {
  ...
  useEffect(() => {
    projectDB.ref(`articles/${articleId}`).on('value', (snapshot) => {
      ...
      setArticle(getArticleInfo(data));
      setIsLoading(false);
    });
  }, []);
  useEffect(() => {
            ...
            isArticleExist ?
              <Card style={{ borderColor: "#888" }} variant="outlined">
                <CardHeader
                    ...
                    localStorage.getItem("currentUserId") === article.articleOwnerId ?
                      <>
                        ...
                        <IconButton
                          ...
                          onClick={() => {
                            setIsDeleting(true);
                            deleteArticleHandler(articleId, article.articleOwnerId, article.articleTags)
                          }}
                  ...
                  title={
                    <NavLink className={styles.linkStyle} to={`/medium-clone/profiles/${article.articleOwnerId}`}>
                      {articleOwnerName}
                    </NavLink>
                  }
              ...
              <div className={styles.messagesContainer}>
                <Typography variant="body1" color="textPrimary" component="p">
                  {feedInfoTranslate.info.articleNotExist}
                </Typography>
              </div>}
        </Paper>
      </div>
