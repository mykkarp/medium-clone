...
export default function ComposedTextField(props) {
  ...
  const sendArticleAndRedirect = () => {
    projectDB.ref('articles/' + Date.now() + "/" + localStorage.getItem("currentUserId")).set({
  ...
  const isTagsCorrect = () => {
    return articleTags.split(" ").every(str => str.length === 0 ? true : str.match(/^\#[a-zA-Z0-9А-Яа-я_]+$/));
  }
  ...
  const articleTagsHandler = (event) => {
    ...
    if (tagsArr[tagsArr.length - 1].length > 16) {
      event.preventDefault();
      return;
    }
